...
static ViString resultCDPArr[]={"SLOT","PDAT","PD1","PD2","PMID","RHO","MACC","PCD","FERR",
								"CERR","TFR","IQOF","IQIM","ACT","SRAT","CHAN","SFAC","CDPR",
								"CDP","EVMR","EVMP","ARCD","CMAP","CSL","MPIC","MTYP",
								"PSYM","PTOT","TOFF",NULL};
static ViString resultCDPArrFSW[]={"SLOT","PDAT","PD1","PD2","PMID","RHO","MACC","PCD","FERR",
								"CERR","TFR","IQOF","IQIM","ACT","SRAT","CHAN","SFAC","CDPR",
								"CDP","EVMR","EVMP","ARCD","CMAP","CSL","MPIC","MTYP",
								"PSYM","PTOT","TOFF",NULL};
...
ViStatus _VI_FUNC rsspecan_SetTDSBSMode(ViSession   instrSession)
{
    ...
    viCheckErr( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_TDBS_MODE, ""));
Error:
...
ViStatus _VI_FUNC rsspecan_SetTDSUEMode(ViSession   instrSession)
{
    ...
    checkErr( Rs_LockSession (instrSession, VI_NULL));
    viCheckErr( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_TDUE_MODE, "")); 
Error:
    (void) Rs_UnlockSession(instrSession, VI_NULL);
    return error;
}
                                                 ...
                                                 ViInt32 measurement)
{
    ...
    if (!( strstr (buffer, "K76") || strstr (buffer, "K77")))    
                                                      ...
                                                      ViInt32 captureLength)
{
    ...
    viCheckParm(rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_INVERT_Q, 
												invertQ), 2, "Invert Q");
	
    viCheckParm(rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_RESULT_SET_COUNT,
												setCount), 3, "Set Count");
	
	viCheckParm(rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_RESULT_SET_VALUE,
												setValue), 4, "Set Value");
	
    if (setCount == 1)
	{
		viCheckParm(rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_CAPTURE_LENGTH,
												captureLength), 5, "Capture Length");
	}
                                                        ...
                                                        ViInt32 slot)
{
    ...
    viCheckParm(rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_RESULT_SEL_CHAN, 
												channel), 2, "Channel");
	
    viCheckParm(rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_RESULT_SEL_SLOT,
												slot), 3, "Slot"); 
                                                        ...
                                                        ViInt32 timeout)
{
    ...
    viCheckErr(rsspecan_SetAttributeViInt32 (instrSession, "", 
				RSSPECAN_ATTR_TDBS_ADAPT_SIGN_AUT_LEVEL_TIME, VI_NULL));
Error:
    if (old_timeout > 0) 
                                                        ...
                                                        ViBoolean rotateCodeChannels)
{
    ...
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_SCODE, 
												scramblingCode), 2, "scramblingCode");
	
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_CELL_SHIFTS,
												midambleShiftCells), 3, "Midamble Shift Cells");
	
	viCheckParm (rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_SYNC_TO_SLOT,
												(ViBoolean) syncTo), 4, "Sync To");
	
    viCheckParm (rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_SYNC_TO_SLOT_ROTATE,
												rotateCodeChannels), 5, "Rotate Code Channels");
                                                         ...
                                                         ViInt32 syncToSlotMode)
{
	...
	viCheckParm(rsspecan_SetAttributeViInt32(instrSession, "",
        RSSPECAN_ATTR_TDUE_STS_MODE, syncToSlotMode), 2, "Sync To Slot Mode");    
                                                                 ...
                                                                 ViReal64 inactiveChannelTreshold)
{
    ...
    viCheckParm (rsspecan_SetAttributeViReal64 (instrSession, "", RSSPECAN_ATTR_TDBS_INACT_CHAN_THR, 
												inactiveChannelTreshold), 2, "InactiveChannel Treshold");
                                                           ...
                                                           ViInt32 codePowerDisplay)
{
    ...
    viCheckParm (rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_NORMALIZE, 
												normalize), 2, "Normalize");
	
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_POWER_DISPLAY,
												codePowerDisplay), 3, "Code Power Display");
                                                           ...
                                                           ViReal64 minimumValue)
{
	...
	viCheckParm (rsspecan_SetAttributeViReal64 (instrSession, "Max",
                                                RSSPECAN_ATTR_TDBS_VERTICAL_SCALE_MAX_MIN,
												maximumValue), 2, "Maximum Value");
                                                        ...
                                                        ViString    channelTable)
{
    ...
    switch(operation){
        case RSSPECAN_VAL_TDS_TABLE_OFF:
            viCheckErr (rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_STATE,
                VI_FALSE)); 
        break;
        case RSSPECAN_VAL_TDS_TABLE_SELECT:
            viCheckErr (rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_STATE,
                VI_TRUE));
            viCheckParm (rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_SEL, 
                 channelTable), 3, "Channel Table");                    
        break;  
        case RSSPECAN_VAL_TDS_TABLE_NEW:
            viCheckParm (rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_NAME, 
                 channelTable), 3, "Channel Table"); 
        break;  
        case RSSPECAN_VAL_TDS_TABLE_DELETE:
            viCheckParm (rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_DEL, 
                 channelTable), 3, "Channel Table");
        break;           
        case RSSPECAN_VAL_TDS_TABLE_COPY:
            viCheckParm (rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_COPY, 
                 channelTable), 3, "Channel Table");
        break;       
                                                ...
                                                ViString    channelTableComment)
{
    ...
    checkErr( Rs_LockSession (instrSession, VI_NULL));
    viCheckParm (rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_COMM, 
                                                            ...
                                                            ViInt32 order)
{
    ...
    checkErr( Rs_LockSession (instrSession, VI_NULL));
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_ORDER, 
                 order), 2, "Order");
Error:
    (void) Rs_UnlockSession(instrSession, VI_NULL);
    return error;
                                                                ...
                                                                ViInt32 maximumofMidableShifts)
{
    ...
    checkErr( Rs_LockSession (instrSession, VI_NULL));
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_CTAB_MSHIFT, 
                                                              ...
                                                              ViInt32 highestModulation)
{
    ...
    viCheckParm(rsspecan_SetAttributeViInt32(instrSession, "",
        RSSPECAN_ATTR_TDBS_CTAB_MMAX, highestModulation), 2, "Highest Modulation");
                                                      ...
                                                      ViInt32 subframes)
{
	...
	viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "",
                                                RSSPECAN_ATTR_TDBS_MEAS_PVT_SWITCH_POINT,
												switchingPoint), 2, "Switching Point"); 
	
	viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "",
                                                RSSPECAN_ATTR_TDBS_MEAS_PVT_SFRAMES,
												subframes), 3, "Subframes");   
Error:
    (void) Rs_UnlockSession(instrSession, VI_NULL);
    return error;
}
                                                      ...
                                                      ViReal64 *results)
{
	...
	checkErr (viQueryf (instrSession, "CONF:CDP:PVT:LIST:RES?\n","%s", buffer));
                                            ...
                                            ViReal64*   result)
{
    ...
    if (rsspecan_invalidViInt32Range (type, RSSPECAN_VAL_TDS_RESULT_SLOT, RSSPECAN_VAL_TDS_RESULT_ARCDE))
    ...
    viCheckErr (viPrintf (instrSession, "CALC:MARK:FUNC:CDP:RES? %s;*OPC\n", 
                resultCDPArr[type]));
    viCheckErr( rsspecan_WaitForOPC (instrSession, timeout)); 
    viCheckErr (viScanf (instrSession, "%le", result));
    checkErr( rsspecan_CheckStatus (instrSession));    
Error:
    (void) Rs_UnlockSession(instrSession, VI_NULL);
    return error;
                                        ...
                                        ViReal64*   result)
{
    ...
    if (rsspecan_invalidViInt32Range (type, RSSPECAN_VAL_TDS_RESULT_SLOT, RSSPECAN_VAL_TDS_RESULT_TOFFSET))
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Type");
    if (!result)
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 4, "Result");        
                                            ...
                                            ViReal64    values[])
{
    ...
    if (rsspecan_invalidViInt32Range (trace, 1, 2))
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Trace");
    if (!actualPoints)
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 4, "Actual Points");          
        
    sprintf (buffer, "TRACE%ld", trace);
    viCheckErr (rsspecan_dataReadTrace (instrSession, 0, buffer, arrayLength, 
                    values, actualPoints));
