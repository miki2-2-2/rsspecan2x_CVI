...
static ViString ADEMTypeArr[] = {"AM","FM","PM","AM:AFSP","FM:AFSP","PM:AFSP","SPEC","AM:REL","AM:REL:AFSP", VI_NULL};
static ViString resultTypeArr[] = {"OFF", "WRIT", "AVER", "MAXH", "MINH", "VIEW", VI_NULL};
...
ViStatus _VI_FUNC rsspecan_ADemodMode(ViSession   instrSession)
{
    ...
    checkErr( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_FMDEM_MODE, VI_NULL));
...
/*    else
    ...
    if (rsspecan_invalidViInt32Range (operatingMode, 0, tmp_mode))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Operating Mode");
    }
    if (rsspecan_invalidViInt32Range (trace, 0, tmp_trace))
    {
    ...
    viCheckErr (viPrintf (instrSession, "%s", buffer));
...
)
{
    ...
    if (strstr(model,"FSL")!=NULL)
        rec_len=480001; 
    if (rsspecan_invalidViReal64Range (sampleRate, 122.0703125, 32.0e+6))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Sample Rate");
    }
    if (rsspecan_invalidViInt32Range (recordLength, 1, rec_len))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 3, "Record Length");
    }
    if (rsspecan_invalidViInt32Range (triggerSource, 0, 1))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 4, "Trigger Source");
    }
    if (rsspecan_invalidViInt32Range (triggerSlope, 0, 1))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 5, "Trigger Slope");
    }
    if (rsspecan_invalidViInt32Range (pretriggerSamples, -65024, 130560))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 6, "Pretrigger Samples");
    }
    if (rsspecan_invalidViInt32Range (numberOfMeas, 0, 32767))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 7, "Number of Measurements");
    }
    ...
    checkErr (viWrite (instrSession, (ViBuf) buffer, (ViUInt32) strlen (buffer), &retCnt));
    ...
    ViInt32 resultTypes[])
{
    ...
    viCheckErr (rsspecan_GetAttributeViString (instrSession, "", RS_ATTR_INSTRUMENT_MODEL, RSSPECAN_IO_BUFFER_SIZE, model));
   
    if (!strstr (buffer, "K7"))
        viCheckErr (RS_ERROR_INSTRUMENT_OPTION);

    if (rsspecan_invalidViInt32Range (demodulationType, 0, 8))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Demodulation Type");
    }
...
)
{
    ...
    switch (filterType)
    {
        ...
        default: viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Filter Type");
    }
                                                        ...
                                                        ViReal64 filterFrequency)
{
	...
	if (rsspecan_invalidViInt32Range (filterType, RSSPECAN_VAL_FMDEM_FILTER_HP, RSSPECAN_VAL_FMDEM_FILTER_LP))
    {
    ...
    {
        viCheckParm( rsspecan_SetAttributeViReal64 (instrSession, "Highpass", 
												RSSPECAN_ATTR_ADEM_FILT_FREQ_MANUAL,
												filterFrequency), 3, "Filter Frequency");
    }
	else
	{
        viCheckParm( rsspecan_SetAttributeViReal64 (instrSession, "Lowpass", 
												RSSPECAN_ATTR_ADEM_FILT_FREQ_MANUAL,
												filterFrequency), 3, "Filter Frequency");
    }
Error:
                                                            ...
                                                            ViBoolean aWeightingFilter)
{
    ...
    viCheckParm( rsspecan_SetAttributeViBoolean (instrSession, "", 
		RSSPECAN_ATTR_ANALOG_DEMOD_FILTER_A_WEIGHTED_ENABLED, aWeightingFilter), 2, "A Weighting Filter");
                                                      ...
                                                      ViReal64 zoomLength)
{
    ...
    viCheckParm( rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_FMDEM_ZOOM_LENGTH_MODE, 
                                               zoomLengthMode), 2, "Zoom Length Mode");
    
	if (zoomLengthMode == RSSPECAN_VAL_MAN)
	{
		viCheckParm( rsspecan_SetAttributeViReal64 (instrSession, "", RSSPECAN_ATTR_FMDEM_ZOOM_LENGTH, 
                                                        ...
                                                        ViReal64 squelchLevel)
{
    ...
    viCheckParm( rsspecan_SetAttributeViBoolean (instrSession, "", 
		RSSPECAN_ATTR_FMDEM_SQUELCH, squelchState), 2, "Squelch State");
    viCheckParm( rsspecan_SetAttributeViReal64 (instrSession, "", 
		RSSPECAN_ATTR_FMDEM_SQUELCH_LEVEL, squelchLevel), 3, "Squelch Level");
                                        ...
                                        ViUInt32 timeout)
{
    ...
    viCheckErr( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_ADEM_SIGNAL_SEARCH, ""));
...
)
{
    ...
    viCheckErr (rsspecan_SetAttributeViString (instrSession, rep_cap,
                                               RSSPECAN_ATTR_ADEM_SELECT_TRACE, NULL));
...
)
{
    ...
    switch (operation)
	{
        case RSSPECAN_VAL_ADEM_PRESET_LOAD:
            viCheckParm( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_ADEM_PRESET_LOAD, 
                standard), 3, "Standard");
        break;
        case RSSPECAN_VAL_ADEM_PRESET_STORE:
            viCheckParm( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_ADEM_PRESET_STORE, 
                standard), 3, "Standard");
        break;
        case RSSPECAN_VAL_ADEM_PRESET_RESTORE:
            viCheckErr( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_ADEM_PRESET_RESTORE, NULL));
        break;
        default:
                                                        ...
                                                        ViBoolean phonesOutput)
{
    ...
    viCheckParm( rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_ANALOG_DEMOD_OUTPUT_STATE, 
																	outputState), 2, "Output State");
	
	viCheckParm( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_ANALOG_DEMOD_OUTPUT_SELECTION, 
															outputSelection), 3, "Output Selection");
	
	viCheckParm( rsspecan_SetAttributeViReal64 (instrSession, "", RSSPECAN_ATTR_ANALOG_DEMOD_OUTPUT_CUTOFF_FREQUENCY, 
														ACCutoffFrequency), 4, "AC Cutoff Frequency");
	
	viCheckParm( rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_ANALOG_DEMOD_PHONE_OUTPUT, 
																	phonesOutput), 5, "Phones Output");
...
ViStatus _VI_FUNC rsspecan_ConfigureADemodAFFullSpan(ViSession   instrSession)
{
    ...
    viCheckErr( rsspecan_SetAttributeViString (instrSession, "", RSSPECAN_ATTR_FMDEM_AF_FULL_SPAN, ""));
    ...
    switch (value)
    {
        ...
        case RSSPECAN_VAL_FMDEM_SRATE:
            viCheckParm( rsspecan_GetAttributeViReal64 (instrSession, "", RSSPECAN_ATTR_FMDEM_SRATE, &l_value),
                                                    3, "Result");
        break;
        default: viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Value");
    }
...
)
{
    ...
    if (rsspecan_invalidViInt32Range (resultType, 1, 4))
    {
...
)
{
    ...
    if (rsspecan_invalidViInt32Range (resultType, 0, 1))
    {
    ...
    switch (resultType)
    {
        case RSSPECAN_VAL_FMDEM_IMM:
            checkErr (viPrintf (instrSession, "*CLS;:SENS:ADEM:FM:OFFS? IMM\n"));
        break;
        case RSSPECAN_VAL_FMDEM_AVG:
            checkErr (viPrintf(instrSession, "*CLS;:SENS:ADEM:FM:OFFS? AVER\n"));
        break;
    }
...
)
{
    ...
    if (rsspecan_invalidViInt32Range (markerNumber, 1, 4))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 2, "Marker Number");
    }
    if (rsspecan_invalidViInt32Range (demodulationType, 0, 2))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 3, "Demodulation Type");
    }
    if (rsspecan_invalidViInt32Range (detectorType, 0, 3))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 4, "Detector Type");
    }
	...
	if (rsspecan_invalidViInt32Range (trace, 1, tmp_trace))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 5, "Trace");
    }

/*
    checkErr (viPrintf (instrSession, "*CLS;:CALC:MARK%ld:FUNC:ADEM:%s:RES%ld? %s\n", markerNumber,
...
)
{
    ...
    if (rsspecan_invalidViInt32Range (measurement, 0, 4))
    {
	...
	if (rsspecan_invalidViInt32Range (trace, 1, tmp_trace))
    {
        viCheckParm (RS_ERROR_INVALID_PARAMETER, 4, "Trace");
    }
