                                                ...
                                                ViInt32     measurement)
{
    ...
    viCheckParm(rsspecan_SetAttributeViInt32(instrSession, "", RSSPECAN_ATTR_TDBS_MEAS, measurement), 2, "Measurement");
Error:
                                                        ...
                                                        ViUInt32   timeout)
{
    ...
    viCheckErr(rsspecan_SetAttributeViInt32(instrSession, "", RSSPECAN_ATTR_TDBS_ADAPT_SIGN_AUT_LEVEL_TIME, VI_NULL));
Error:
    if (old_timeout > 0) 
                                               ...
                                               ViUInt32      timeout)
{
    ...
    viCheckErr(rsspecan_SetAttributeViString(instrSession, "", RSSPECAN_ATTR_TDBS_CDP_LEV_ADJUST, VI_NULL));
Error:
    if (old_timeout >= 0)    
        rsspecan_SetOPCTimeout (instrSession, old_timeout);
    (void) Rs_UnlockSession(instrSession, VI_NULL);
                                                    ...
                                                    ViBoolean   invertQ)
{
    ...
    checkErr( Rs_LockSession (instrSession, VI_NULL));
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_STANDARD, 
                 standard), 2, "Standard");
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_SCODE, 
                 scramblingCode), 3, "Scrambling Code");    
    ...
    if (tmp_state == VI_FALSE)
    {
        viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_CELL_SHIFTS, 
                     midableShiftCells), 4, "Midable Shifts");  
    }                
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_SIDE_BAND, 
    ...
    viCheckParm (rsspecan_SetAttributeViReal64 (instrSession, "", RSSPECAN_ATTR_TDBS_INACT_CHAN_THR, 
                 inactiveChannelTreshold), 8, "Inactive Channel Treshold"); 
                                                         ...
                                                         ViInt32 syncToSlotMode)
{
	...
	viCheckParm(rsspecan_SetAttributeViInt32(instrSession, "",
        RSSPECAN_ATTR_TDBS_STS_MODE, syncToSlotMode), 2, "Sync To Slot Mode");
                                                                       ...
                                                                       ViBoolean rotateCodeChannels)
{
	...
	viCheckParm(rsspecan_SetAttributeViBoolean(instrSession, "",
        RSSPECAN_ATTR_TDBS_SYNC_TO_SLOT_ROTATE, rotateCodeChannels), 2, "Rotate Code Channels");
...
)
{
    ...
    viCheckParm (rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_CPICH_STATE, 
                 userDefinedCPICH), 2, "User Defined CPICH");

    if (userDefinedCPICH) {
    
        viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_CPICH_CODE, 
                 cpichCode), 3, "CPICH Code");

        viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_CPICH_PATTERN, 
                                                        ...
                                                        ViBoolean highDynamicState)
{
    ...
    checkErr( Rs_LockSession (instrSession, VI_NULL));
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_MEAS_PVT_SWITCH_POINT, 
                 switchingPoint), 2, "Switching Point");
    viCheckParm (rsspecan_SetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_MEAS_PVT_SFRAMES, 
                 subframes), 3, "Subframes");    
    viCheckParm (rsspecan_SetAttributeViBoolean (instrSession, "", RSSPECAN_ATTR_TDBS_MEAS_PVT_HDYN, 
                 highDynamicState), 4, "High Dynamic State");                  
Error:
    (void) Rs_UnlockSession(instrSession, VI_NULL);
    return error;
}
                                                                    ...
                                                                    ViInt32*    result)
{
    ...
    checkErr (rsspecan_GetOPCTimeout (instrSession, &old_timeout));
    checkErr (rsspecan_SetOPCTimeout (instrSession, timeout));
    viCheckParm (rsspecan_GetAttributeViInt32 (instrSession, "", RSSPECAN_ATTR_TDBS_ADJ_RLEV_QUERY, 
                 result), 2, "Result"); 
Error:
    if (old_timeout >= 0)    
        rsspecan_SetOPCTimeout (instrSession, old_timeout);
